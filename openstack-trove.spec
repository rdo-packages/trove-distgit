...
%global excluded_brs doc8 bandit pre-commit hacking flake8-import-order bashate astroid couchdb nosexcover openstack.nose-plugin os-api-ref proboscis pylint psycopg2-binary
...
for pkg in %{excluded_brs}; do
  for reqfile in doc/requirements.txt test-requirements.txt backup/requirements.txt; do
    if [ -f $reqfile ]; then
      sed -i /^${pkg}.*/d $reqfile
    fi
  done
